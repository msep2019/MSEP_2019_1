setting as below to add dependencies:

name := "Test SVM"

version := "1.0"

scalaVersion := "2.12.8"

libraryDependencies += "org.apache.spark" % "spark-mllib_2.12" % "2.4.2"

libraryDependencies += "org.apache.spark" % "spark-mllib-local_2.12" % "2.4.2"

libraryDependencies += "org.apache.spark" % "spark-core_2.12" % "2.4.2"

libraryDependencies += "org.apache.hadoop" % "hadoop-common" % "2.9.2"
	
libraryDependencies += "io.airlift" % "aircompressor" % "0.10"

libraryDependencies += "org.antlr" % "antlr4-runtime" % "4.7"
	
libraryDependencies += "net.sourceforge.f2j" % "arpack_combined_all" % "0.1"
	
libraryDependencies += "org.apache.arrow" % "arrow-format" % "0.10.0"
	
libraryDependencies += "org.apache.arrow" % "arrow-memory" % "0.10.0"
	
libraryDependencies += "org.apache.arrow" % "arrow-vector" % "0.10.0"
	
libraryDependencies += "dk.brics.automaton" % "automaton" % "1.11-8"
	
libraryDependencies += "org.apache.avro" % "avro-mapred" % "1.8.2"
	
libraryDependencies += "com.twitter" % "chill_2.12" % "0.9.3"
	
libraryDependencies += "com.twitter" % "chill-java" % "0.9.3"
	
libraryDependencies += "commons-codec" % "commons-codec" % "1.10"
	
libraryDependencies += "org.codehaus.janino" % "commons-compiler" % "3.0.9"
	
libraryDependencies += "org.apache.commons" % "commons-crypto" % "1.0.0"
	
libraryDependencies += "org.apache.commons" % "commons-lang3" % "3.5"
	
libraryDependencies += "commons-lang" % "commons-lang" % "2.6"
	
libraryDependencies += "org.apache.commons" % "commons-math3" % "3.4.1"
	
libraryDependencies += "commons-net" % "commons-net" % "3.1"
	
libraryDependencies += "com.ning" % "compress-lzf" % "1.0.3"
	
libraryDependencies += "com.github.fommil.netlib" % "core" % "1.1.2"
	
libraryDependencies += "com.vlkan" % "flatbuffers" % "1.2.0-3f79e055"
	
libraryDependencies += "com.github.mifmif" % "generex" % "1.0.1"
	
libraryDependencies += "org.glassfish.hk2" % "hk2-api" % "2.4.0"
	
libraryDependencies += "org.glassfish.hk2" % "hk2-locator" % "2.4.0-b34"
	
libraryDependencies += "org.glassfish.hk2" % "hk2-utils" % "2.4.0-b34"
	
libraryDependencies += "com.carrotsearch" % "hppc" % "0.7.2"
	
libraryDependencies += "org.apache.ivy" % "ivy" % "2.4.0"
	
libraryDependencies += "com.fasterxml.jackson.core" % "jackson-annotations" % "2.6.7"
	
libraryDependencies += "com.fasterxml.jackson.core" % "jackson-core" % "2.6.7"
	
libraryDependencies += "com.fasterxml.jackson.core" % "jackson-databind" % "2.6.7.1"
	
libraryDependencies += "com.fasterxml.jackson.dataformat" % "jackson-dataformat-yaml" % "2.6.7"
	
libraryDependencies += "com.fasterxml.jackson.module" % "jackson-module-jaxb-annotations" % "2.6.7"
	
libraryDependencies += "com.fasterxml.jackson.module" % "jackson-module-paranamer" % "2.7.9"
	
libraryDependencies += "com.fasterxml.jackson.module" % "jackson-module-scala_2.12" % "2.6.7.1"
	
libraryDependencies += "org.codehaus.janino" % "janino" % "3.0.9"
	
libraryDependencies += "org.javassist" % "javassist" % "3.18.1-GA"
	
libraryDependencies += "javax.annotation" % "javax.annotation-api" % "1.2"
	
libraryDependencies += "org.glassfish.hk2.external" % "javax.inject" % "2.4.0-b34"
	
libraryDependencies += "javax.servlet" % "javax.servlet-api" % "3.1.0"
	
libraryDependencies += "javax.ws.rs" % "javax.ws.rs-api" % "2.0.1"
	
libraryDependencies += "org.slf4j" % "jcl-over-slf4j" % "1.7.16"
	
libraryDependencies += "org.glassfish.jersey.core" % "jersey-client" % "2.22.2"
	
libraryDependencies += "org.glassfish.jersey.core" % "jersey-common" % "2.22.2"
	
libraryDependencies += "org.glassfish.jersey.containers" % "jersey-container-servlet" % "2.22.2"
	
libraryDependencies += "org.glassfish.jersey.containers" % "jersey-container-servlet-core" % "2.22.2"
	
libraryDependencies += "org.glassfish.jersey.bundles.repackaged" % "jersey-guava" % "2.22.2"
	
libraryDependencies += "org.glassfish.jersey.media" % "jersey-media-jaxb" % "2.22.2"
	
libraryDependencies += "org.glassfish.jersey.core" % "jersey-server" % "2.22.2"
	
libraryDependencies += "joda-time" % "joda-time" % "2.9.3"
	
libraryDependencies += "org.json4s" % "json4s-ast_2.12" % "3.5.3"
	
libraryDependencies += "org.json4s" % "json4s-core_2.12" % "3.5.3"
	
libraryDependencies += "org.json4s" % "json4s-jackson_2.12" % "3.5.3"
	
libraryDependencies += "org.json4s" % "json4s-scalap_2.12" % "3.5.3"
	
libraryDependencies += "com.google.code.findbugs" % "jsr305" % "1.3.9"
	
libraryDependencies += "com.github.rwl" % "jtransforms" % "2.4.0"
	
libraryDependencies += "org.slf4j" % "jul-to-slf4j" % "1.7.16"
	
libraryDependencies += "com.esotericsoftware" % "kryo-shaded" % "4.0.2"
	
libraryDependencies += "io.fabric8" % "kubernetes-client" % "4.1.2"
	
libraryDependencies += "io.fabric8" % "kubernetes-model" % "4.1.2"
	
libraryDependencies += "io.fabric8" % "kubernetes-model-common" % "4.1.2"
	
libraryDependencies += "org.fusesource.leveldbjni" % "leveldbjni-all" % "1.8"
	
libraryDependencies += "com.squareup.okhttp3" % "logging-interceptor" % "3.12.0"
	
libraryDependencies += "org.lz4" % "lz4-java" % "1.4.0"
	
libraryDependencies += "org.typelevel" % "machinist_2.12" % "0.6.1"
	
libraryDependencies += "org.typelevel" % "macro-compat_2.12" % "1.1.1"
	
libraryDependencies += "org.apache.mesos" % "mesos" % "1.4.0"
	
libraryDependencies += "io.dropwizard.metrics" % "metrics-core" % "3.1.5"
	
libraryDependencies += "io.dropwizard.metrics" % "metrics-graphite" % "3.1.5"
	
libraryDependencies += "io.dropwizard.metrics" % "metrics-json" % "3.1.5"
	
libraryDependencies += "io.dropwizard.metrics" % "metrics-jvm" % "3.1.5"
	
libraryDependencies += "com.esotericsoftware" % "minlog" % "1.3.0"
	
libraryDependencies += "io.netty" % "netty" % "3.9.9.Final"
	
libraryDependencies += "io.netty" % "netty-all" % "4.1.17.Final"
	
libraryDependencies += "org.objenesis" % "objenesis" % "2.5.1"
	
libraryDependencies += "com.squareup.okhttp3" % "okhttp" % "3.8.1"
	
libraryDependencies += "com.squareup.okio" % "okio" % "1.13.0"
	
libraryDependencies += "net.sf.opencsv" % "opencsv" % "2.3"
	
libraryDependencies += "org.apache.orc" % "orc-core" % "1.5.5"
	
libraryDependencies += "org.apache.orc" % "orc-mapreduce" % "1.5.5"
	
libraryDependencies += "org.apache.orc" % "orc-shims" % "1.5.5"
	
libraryDependencies += "oro" % "oro" % "2.0.8"
	
libraryDependencies += "org.glassfish.hk2" % "osgi-resource-locator" % "1.0.1"
	
libraryDependencies += "com.thoughtworks.paranamer" % "paranamer" % "2.8"
	
libraryDependencies += "org.apache.parquet" % "parquet-column" % "1.10.1"
	
libraryDependencies += "org.apache.parquet" % "parquet-common" % "1.10.1"
	
libraryDependencies += "org.apache.parquet" % "parquet-encoding" % "1.10.1"
	
libraryDependencies += "org.apache.parquet" % "parquet-format" % "2.4.0"
	
libraryDependencies += "org.apache.parquet" % "parquet-hadoop" % "1.10.1"
	
libraryDependencies += "org.apache.parquet" % "parquet-jackson" % "1.10.1"
	
libraryDependencies += "net.sf.py4j" % "py4j" % "0.10.7"
	
libraryDependencies += "net.razorvine" % "pyrolite" % "4.13"
	
libraryDependencies += "org.roaringbitmap" % "RoaringBitmap" % "0.7.45"
	
libraryDependencies += "org.scala-lang" % "scala-compiler" % "2.12.8"
	
libraryDependencies += "org.scala-lang" % "scala-library" % "2.12.8"
	
libraryDependencies += "org.scala-lang.modules" % "scala-parser-combinators_2.12" % "1.1.0"
	
libraryDependencies += "org.scala-lang" % "scala-reflect" % "2.12.8"
	
libraryDependencies += "org.scala-lang.modules" % "scala-xml_2.12" % "1.0.5"
	
libraryDependencies += "com.chuusai" % "shapeless_2.12" % "2.3.2"
	
libraryDependencies += "org.roaringbitmap" % "shims" % "0.7.45"
	
libraryDependencies += "org.yaml" % "snakeyaml" % "1.15"
	
libraryDependencies += "org.xerial.snappy" % "snappy-java" % "1.1.7.3"
	
libraryDependencies += "org.apache.spark" % "spark-catalyst_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-graphx_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-kubernetes_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-kvstore_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-launcher_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-mesos_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-network-common_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-network-shuffle_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-repl_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-sketch_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-sql_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-streaming_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-tags_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-unsafe_2.12" % "2.4.2"
	
libraryDependencies += "org.apache.spark" % "spark-yarn_2.12" % "2.4.2"
	
libraryDependencies += "org.spire-math" % "spire_2.12" % "0.13.0"
	
libraryDependencies += "org.spire-math" % "spire-macros_2.12" % "0.13.0"
	
libraryDependencies += "com.clearspring.analytics" % "stream" % "2.7.0"
	
libraryDependencies += "com.univocity" % "univocity-parsers" % "2.7.3"
	
libraryDependencies += "javax.validation" % "validation-api" % "1.1.0.Final"
	
libraryDependencies += "org.apache.xbean" % "xbean-asm6-shaded" % "4.8"
	
libraryDependencies += "io.fabric8" % "zjsonpatch" % "0.3.0"
	
libraryDependencies += "com.github.luben" % "zstd-jni" % "1.3.2-2"
	
libraryDependencies += "org.slf4j" % "slf4j-api" % "1.7.25"
	
libraryDependencies += "org.slf4j" % "slf4j-log4j12" % "1.7.25"
	
libraryDependencies += "log4j" % "log4j" % "1.2.17"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-mapreduce-client-common" % "2.9.2"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-mapreduce-client-app" % "2.9.2"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-mapreduce-client-core" % "2.9.2"
	
libraryDependencies += "commons-configuration" % "commons-configuration" % "1.6"
	
libraryDependencies += "commons-collections" % "commons-collections" % "3.2.2"
	
libraryDependencies += "javax.activation" % "activation" % "1.1"
	
libraryDependencies += "org.apache.directory.server" % "apacheds-i18n" % "2.0.0-M15"
	
libraryDependencies += "org.apache.directory.server" % "apacheds-kerberos-codec" % "2.0.0-M15"
	
libraryDependencies += "org.apache.directory.api" % "api-asn1-api" % "1.0.0-M20"
	
libraryDependencies += "org.apache.directory.api" % "api-util" % "1.0.0-M20"
	
libraryDependencies += "asm" % "asm" % "3.2"
	
libraryDependencies += "org.apache.avro" % "avro" % "1.7.7"
	
libraryDependencies += "commons-beanutils" % "commons-beanutils" % "1.7.0"
	
libraryDependencies += "commons-beanutils" % "commons-beanutils-core" % "1.8.0"
	
libraryDependencies += "commons-cli" % "commons-cli" % "1.2"
	
libraryDependencies += "commons-codec" % "commons-codec" % "1.4"
	
libraryDependencies += "org.apache.commons" % "commons-compress" % "1.4.1"
	
libraryDependencies += "commons-digester" % "commons-digester" % "1.8"
	
libraryDependencies += "commons-io" % "commons-io" % "2.4"
	
libraryDependencies += "org.apache.commons" % "commons-lang3" % "3.4"
	
libraryDependencies += "commons-lang" % "commons-lang" % "2.6"
	
libraryDependencies += "commons-logging" % "commons-logging" % "1.1.3"
	
libraryDependencies += "org.apache.commons" % "commons-math3" % "3.1.1"
	
libraryDependencies += "commons-net" % "commons-net" % "3.1"
	
libraryDependencies += "org.apache.curator" % "curator-client" % "2.7.1"
	
libraryDependencies += "org.apache.curator" % "curator-framework" % "2.7.1"
	
libraryDependencies += "org.apache.curator" % "curator-recipes" % "2.7.1"
	
libraryDependencies += "com.google.code.gson" % "gson" % "2.2.4"
	
libraryDependencies += "com.google.guava" % "guava" % "11.0.2"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-annotations" % "2.9.2"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-auth" % "2.9.2"
	
libraryDependencies += "org.hamcrest" % "hamcrest-core" % "1.3"
	
libraryDependencies += "org.apache.htrace" % "htrace-core4" % "4.1.0-incubating"
	
libraryDependencies += "org.apache.httpcomponents" % "httpclient" % "4.5.2"
	
libraryDependencies += "org.apache.httpcomponents" % "httpcore" % "4.4.4"
	
libraryDependencies += "org.codehaus.jackson" % "jackson-core-asl" % "1.9.13"
	
libraryDependencies += "org.codehaus.jackson" % "jackson-jaxrs" % "1.9.13"
	
libraryDependencies += "org.codehaus.jackson" % "jackson-mapper-asl" % "1.9.13"
	
libraryDependencies += "org.codehaus.jackson" % "jackson-xc" % "1.9.13"
	
libraryDependencies += "com.jamesmurty.utils" % "java-xmlbuilder" % "0.4"
	
libraryDependencies += "javax.xml.bind" % "jaxb-api" % "2.2.2"
	
libraryDependencies += "com.sun.xml.bind" % "jaxb-impl" % "2.2.3-1"
	
libraryDependencies += "com.github.stephenc.jcip" % "jcip-annotations" % "1.0-1"
	
libraryDependencies += "com.sun.jersey" % "jersey-core" % "1.9"
	
libraryDependencies += "com.sun.jersey" % "jersey-json" % "1.9"
	
libraryDependencies += "com.sun.jersey" % "jersey-server" % "1.9"
	
libraryDependencies += "net.java.dev.jets3t" % "jets3t" % "0.9.0"
	
libraryDependencies += "org.codehaus.jettison" % "jettison" % "1.1"
	
libraryDependencies += "org.mortbay.jetty" % "jetty" % "6.1.26"
	
libraryDependencies += "org.mortbay.jetty" % "jetty-sslengine" % "6.1.26"
	
libraryDependencies += "org.mortbay.jetty" % "jetty-util" % "6.1.26"
	
libraryDependencies += "com.jcraft" % "jsch" % "0.1.54"
	
libraryDependencies += "net.minidev" % "json-smart" % "1.3.1"
	
libraryDependencies += "javax.servlet.jsp" % "jsp-api" % "2.1"
	
libraryDependencies += "com.google.code.findbugs" % "jsr305" % "3.0.0"
	
libraryDependencies += "junit" % "junit" % "4.11"
	
libraryDependencies += "org.mockito" % "mockito-all" % "1.8.5"
	
libraryDependencies += "io.netty" % "netty" % "3.6.2.Final"
	
libraryDependencies += "com.nimbusds" % "nimbus-jose-jwt" % "4.41.1"
	
libraryDependencies += "com.thoughtworks.paranamer" % "paranamer" % "2.3"
	
libraryDependencies += "com.google.protobuf" % "protobuf-java" % "2.5.0"
	
libraryDependencies += "javax.servlet" % "servlet-api" % "2.5"
	
libraryDependencies += "org.xerial.snappy" % "snappy-java" % "1.0.5"
	
libraryDependencies += "org.codehaus.woodstox" % "stax2-api" % "3.1.4"
	
libraryDependencies += "javax.xml.stream" % "stax-api" % "1.0-2"
	
libraryDependencies += "com.fasterxml.woodstox" % "woodstox-core" % "5.0.3"
	
libraryDependencies += "xmlenc" % "xmlenc" % "0.52"
	
libraryDependencies += "org.tukaani" % "xz" % "1.0"
	
libraryDependencies += "org.apache.zookeeper" % "zookeeper" % "3.4.6"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-hdfs" % "2.9.2"
	
libraryDependencies += "org.apache.hadoop" % "hadoop-hdfs-client" % "2.9.2"
	
libraryDependencies += "org.scalanlp" % "breeze_2.12" % "0.13.2"
	
libraryDependencies += "org.scalanlp" % "breeze-macros_2.12" % "0.13.2"
