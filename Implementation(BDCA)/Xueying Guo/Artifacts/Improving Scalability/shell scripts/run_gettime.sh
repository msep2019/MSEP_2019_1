...
#usage: ./run_gettime.sh <number_of_executor> <dataset_type>
#example: ./run_gettime.sh 2 kdd
...
LOG_FILE="${LOG_PATH}/${2}_${1}executors_${NAME}_${timestamp}.log"
...
if [ $? != 0 ]; then
 echo "failed to login MASTER NODE"
 exit 1
fi
...
cmd="$SPARK_HOME/bin/spark-submit --master yarn --deploy-mode cluster --num-executors ${NUM_EXECUTOR} --executor-cores 1 $PACKAGE_NAME"
