...
#     continue
   ...
   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> usr average = ${usravg} " >> ${REULST_FILE}
   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> sys average = ${sysavg} " >> ${REULST_FILE}
   ...
   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> training time = ${trainTtl} ms " >> ${REULST_FILE}
   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> testing time = ${testTtl} ms " >> ${REULST_FILE}
   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> total map reduce = ${total} seconds " >> ${REULST_FILE}
#   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> total over 35% = ${durttl} seconds " >> ${REULST_FILE}
#   echo "<${algorithm}> <${dataset}> <${LINE}:${VALUE}> number of nodes over 35% = ${count} " >> ${REULST_FILE}
   echo "  " >> ${REULST_FILE}

   echo "${algorithm},${dataset},${LINE},${VALUE},${cpuavg},${usravg},${sysavg},${memoryvag},${dsk_readavg},${dsk_writavg},${net_recvavg},${net_sendavg},${io_readavg},${io_writeavg},${blo_io_readavg},${blo_io_writeavg},${trainTtl},${total},${testTtl}" >> ${REULST_CSV}
